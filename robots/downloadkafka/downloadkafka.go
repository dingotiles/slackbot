...
func init() {
	r := &bot{}
	robots.RegisterRobot("download-kafka", r)
}
...
func (r bot) Run(p *robots.Payload) string {
	...
	return "Baby dingos are building a URL just for you for the latest Dingo Kafka tile."
}
...
func (r bot) DeferredAction(p *robots.Payload) {
	...
	webbookResponse := &robots.IncomingWebhook{
		...
		Channel:     p.ChannelID,
		Username:    "dingobot",
		Text:        fmt.Sprintf("Another happy %s v%s tile sent on its way to a new home! (via `/download-kafka`)", productName, version),
		IconEmoji:   ":dingo:",
		UnfurlLinks: false,
		Parse:       robots.ParseStyleFull,
	}
