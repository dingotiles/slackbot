...
func init() {
	r := &bot{}
	robots.RegisterRobot("download-haproxy", r)
}
...
func (r bot) Run(p *robots.Payload) string {
	...
	return "Baby dingos are building a URL just for you for the latest Dingo HAProxy tile."
}
...
func (r bot) DeferredAction(p *robots.Payload) {
	...
	productName := "Dingo HAProxy"
	productLabel := "dingo-haproxy"
	...
	webbookResponse := &robots.IncomingWebhook{
		...
		Channel:     p.ChannelID,
		Username:    "dingobot",
		Text:        fmt.Sprintf("Another happy %s v%s tile sent on its way to a new home! (via `/download-haproxy`)", productName, version),
		IconEmoji:   ":dingo:",
		UnfurlLinks: false,
		Parse:       robots.ParseStyleFull,
	}
...
func (r bot) Description() string {
	...
	return "Fetch URL to download latest Dingo HAProxy tile."
}
