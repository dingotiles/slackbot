...
func init() {
	r := &bot{}
	robots.RegisterRobot("download-secrets", r)
}
...
func (r bot) Run(p *robots.Payload) string {
	...
	return "Baby dingos are building a URL just for you for the latest Dingo Secrets tile."
}
...
func (r bot) DeferredAction(p *robots.Payload) {
	...
	productName := "Dingo Secrets"
	productLabel := "dingo-secrets"
	...
	webbookResponse := &robots.IncomingWebhook{
		...
		Channel:     p.ChannelID,
		Username:    "dingobot",
		Text:        fmt.Sprintf("Another happy %s v%s tile sent on its way to a new home! (via `/download-secrets`)", productName, version),
		IconEmoji:   ":dingo:",
		UnfurlLinks: false,
		Parse:       robots.ParseStyleFull,
	}
...
func (r bot) Description() string {
	...
	return "Fetch URL to download latest Dingo Secrets tile."
}
