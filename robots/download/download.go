...
func init() {
	r := &bot{}
	robots.RegisterRobot("download", r)
}
...
func (r bot) DeferredAction(p *robots.Payload) {
	...
	productName := "Dingo S3"
	productLabel := "dingo-s3"
	...
	response := &robots.SlashCommandResponse{
		...
		Channel:     p.ChannelID,
		Username:    "dingobot",
		Text:        fmt.Sprintf("Download %s %s tile at %s", productName, version, url),
		IconEmoji:   ":dingo:",
		UnfurlLinks: false,
	...
	webbookResponse := &robots.IncomingWebhook{
		...
		Channel:     p.ChannelID,
		Username:    "dingobot",
		Text:        fmt.Sprintf("Another happy %s %s tile sent on its way to a new home! (via `/download`)", productName, version),
		IconEmoji:   ":dingo:",
		UnfurlLinks: false,
		Parse:       robots.ParseStyleFull,
	}
...
func (r bot) Description() string {
	...
	return "Fetch URL to download latest Dingo S3 tile."
}
